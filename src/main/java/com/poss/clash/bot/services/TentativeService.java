...
public class TentativeService {
  ...
  public Flux<TentativeQueue> retrieveTentativeQueues(
    ...
    if (StringUtils.isNotBlank(discordId)
      ...
      tentativeQueueFlux = tentativeDao.findByTentativeId_ServerId_AndTentativeId_TournamentId_TournamentName_AndTentativeId_TournamentId_TournamentDay_AndDiscordIdsContaining(
          serverId, tournamentName, tournamentDay, discordId);
    } else if (StringUtils.isNotBlank(discordId)
               && StringUtils.isNotBlank(serverId)
               && StringUtils.isNotBlank(tournamentName)) {
      tentativeQueueFlux = tentativeDao.findByTentativeId_ServerId_AndTentativeId_TournamentId_TournamentName_AndDiscordIdsContaining(
          serverId, tournamentName, discordId);
    } else if (StringUtils.isNotBlank(discordId)
               && StringUtils.isNotBlank(tournamentName)
               && StringUtils.isNotBlank(tournamentDay)) {
      tentativeQueueFlux = tentativeDao.findByTentativeId_TournamentId_TournamentName_AndTentativeId_TournamentId_TournamentDay_AndDiscordIdsContaining(
          tournamentName, tournamentDay, discordId);
    } else if (StringUtils.isNotBlank(discordId)
               && StringUtils.isNotBlank(tournamentName)
               && StringUtils.isNotBlank(tournamentDay)) {
      tentativeQueueFlux = tentativeDao.findByTentativeId_TournamentId_TournamentName_AndTentativeId_TournamentId_TournamentDay_AndDiscordIdsContaining(
          tournamentName, tournamentDay, discordId);
               ...
               && StringUtils.isNotBlank(serverId)
               && StringUtils.isNotBlank(tournamentDay)) {
      tentativeQueueFlux = tentativeDao.findByTentativeId_ServerId_AndTentativeId_TournamentId_TournamentDay_AndDiscordIdsContaining(
          serverId, tournamentDay, discordId);
    } else if (StringUtils.isNotBlank(discordId)
               && StringUtils.isNotBlank(serverId)
               && StringUtils.isNotBlank(tournamentDay)) {
      tentativeQueueFlux = tentativeDao.findByTentativeId_ServerId_AndTentativeId_TournamentId_TournamentDay_AndDiscordIdsContaining(
          serverId, tournamentDay, discordId);
               ...
               && StringUtils.isNotBlank(tournamentName)
               && StringUtils.isNotBlank(tournamentDay)) {
      tentativeQueueFlux = tentativeDao.findByTentativeId_ServerId_AndTentativeId_TournamentId_TournamentName_AndTentativeId_TournamentId_TournamentDay(
      ...
      tentativeQueueFlux = tentativeDao.findByTentativeId_ServerId_AndDiscordIdsContaining(serverId, discordId);
               ...
               && StringUtils.isNotBlank(tournamentDay)) {
      tentativeQueueFlux = tentativeDao.findByTentativeId_TournamentId_TournamentDay_AndDiscordIdsContaining(
          tournamentDay, discordId);
    } else if (StringUtils.isNotBlank(discordId)
               && StringUtils.isNotBlank(tournamentName)) {
      tentativeQueueFlux = tentativeDao.findByTentativeId_TournamentId_TournamentName_AndDiscordIdsContaining(
          tournamentName, discordId);
    } else if (StringUtils.isNotBlank(serverId)
               && StringUtils.isNotBlank(tournamentName)) {
      tentativeQueueFlux = tentativeDao.findByTentativeId_ServerId_AndTentativeId_TournamentId_TournamentName(serverId,
                                                                                                              tournamentName);
    } else if (StringUtils.isNotBlank(serverId)
               && StringUtils.isNotBlank(tournamentDay)) {
      tentativeQueueFlux = tentativeDao.findByTentativeId_ServerId_AndTentativeId_TournamentId_TournamentDay(serverId,
                                                                                                             tournamentDay);
    } else if (StringUtils.isNotBlank(serverId)) {
      tentativeQueueFlux = tentativeDao.findByTentativeId_ServerId(serverId);
    } else if (StringUtils.isNotBlank(tournamentName)
               && StringUtils.isNotBlank(tournamentDay)) {
      tentativeQueueFlux = tentativeDao.findByTentativeId_TournamentId_TournamentName_AndTentativeId_TournamentId_TournamentDay(
  ...
  public Mono<TentativeQueue> assignUserToTentativeQueue(String discordId, TentativeQueue tentativeQueue) {
    tentativeQueue
        ...
        .add(discordId);
    return tentativeDao
        .updateByTentativeId_TentativeId(tentativeQueue
                                             .getTentativeId()
  ...
  public Mono<TentativeQueue> removeUserFromTentativeQueue(String discordId, String tentativeId) {
    return tentativeDao
        .findByTentativeId_TentativeId(tentativeId)
        .flatMap(tentativeQueue -> tentativeDao
            .removeByTentativeId_TentativeId(tentativeId, discordId)
            .map(count -> {
              tentativeQueue
                  .getDiscordIds()
                  .remove(discordId);
  ...
  public Mono<Boolean> doesServerAlreadyHaveATentativeQueueForTournament(
  ...
  public Flux<TentativeQueue> retrieveTentativeQueuesBasedOnTournaments(List<TournamentId> tournamentIds) {
    return tentativeDao.findAllByTentativeId_TournamentIdIsIn(tournamentIds);
  }
