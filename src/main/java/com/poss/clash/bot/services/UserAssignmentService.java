...
public class UserAssignmentService {
  ...
  public Mono<ClashTeam> assignUserToTeam(String discordId, Role role, String clashTeamId) {
    return teamService
        ...
        .flatMap(clashTeam -> userAssociationService
            ...
            .flatMap(userAssociation -> {
              if (StringUtils.equals(userAssociation.getTeamId(), clashTeamId)) {
                return Mono.just(UserAssociation
                                     .builder()
                                     .build());
              }
              ...
              if (StringUtils.isNotBlank(userAssociation.getTeamId())) {
                return this.teamService
                    .removeUserFromTeam(userAssociation.getTeamId(), discordId)
              ...
              return tentativeService
                  .removeUserFromTentativeQueue(discordId, userAssociation.getTentativeId())
                  .flatMap(updatedTentativeQueue -> teamSource.sendTentativeQueueRemovedEvent(
                      tentativeMapper.tentativeQueueToTentative(updatedTentativeQueue)))
                  .thenReturn(updatedUserAssc);
            })
        ...
        .flatMap(tuple -> teamService
            .upsertClashTeam(tuple.getT1())
            .log()
        ...
        .switchIfEmpty(Mono.error(
            new ClashBotDbException(MessageFormat.format("No Team found with id {0}", clashTeamId),
                                    HttpStatus.NOT_FOUND)));
  }

  private ClashTeam swapRole(String discordId, Role role, ClashTeam clashTeam) {
    Optional<Map.Entry<Role, BasePlayerRecord>> existingUserRole = clashTeam
        .getPositions()
        .entrySet()
        .stream()
        .filter((kv) -> StringUtils.equals(discordId, kv
            .getValue()
            .getDiscordId()))
        .findFirst();
    existingUserRole.ifPresent((foundRole) -> clashTeam
        .getPositions()
        .remove(foundRole.getKey()));
    clashTeam
        .getPositions()
  ...
  private static UserAssociation buildUserAssociationForTeam(
      String discordId, String clashTeamId, ClashTeam clashTeam
  ) {
  ...
  protected ClashTeam validateTeamAvailability(Role role, ClashTeam returnedClashTeam) {
    if (returnedClashTeam
        .getPositions()
        .containsKey(role)) {
      throw new ClashBotDbException(
          MessageFormat.format("Role {0} already taken on Team {1}", role, returnedClashTeam.getTeamId()),
          HttpStatus.BAD_REQUEST);
    }
    return returnedClashTeam;
  }

  public Mono<ClashTeam> createTeamAndAssignUser(
      Map<Role, String> positions, String teamName, String discordServerId, String tournamentName, String tournamentDay
  ) {
    return tournamentService
        .isTournamentActive(tournamentName, tournamentDay)
        .flatMapMany(active -> {
          if (!active) {
        ...
        .flatMap(entry -> this.userAssociationService
            ...
            .flatMap(assc -> {
              if (StringUtils.isNotBlank(assc.getTeamId())) {
                return teamService
                    .removeUserFromTeam(assc.getTeamId(), assc
              ...
              return tentativeService
                  .removeUserFromTentativeQueue(assc
                                                    .getUserAssociationKey()
                                                    .getDiscordId(), assc.getTentativeId())
                  .log()
        ...
        .collectList()
        .flatMap(assc -> teamService
            .createClashTeam(createNewTeam(positions, teamName, discordServerId, tournamentName, tournamentDay))
            .flatMap(userService::enrichClashTeamWithUserDetails)
            .flatMap((createdTeam) -> teamSource
                .sendTeamCreateEvent(teamMapper.clashTeamToTeam(createdTeam))
                .thenReturn(createdTeam))
            .map(createdTeam -> associateUsersToTeam(assc, createdTeam)))
        .flatMap(tuple -> Flux
            .fromIterable(tuple.getT2())
  ...
  private Tuple2<ClashTeam, List<UserAssociation>> associateUsersToTeam(
      List<UserAssociation> assc, ClashTeam createdTeam
  ) {
    List<UserAssociation> updatedAssociations = assc
        .stream()
        .peek(ua -> {
          ua.setTeamId(createdTeam
                           .getTeamId()
                           .getId());
  ...
  private UserAssociation buildBaseAssociation(
      String discordServerId, String tournamentName, String tournamentDay, String id
  ) {
    return UserAssociation
        .builder()
        .userAssociationKey(UserAssociationKey
  ...
  private ClashTeam createNewTeam(
      Map<Role, String> positions, String teamName, String discordServerId, String tournamentName, String tournamentDay
  ) {
    Map<Role, BasePlayerRecord> positionsMap = positions
        .entrySet()
        .stream()
  ...
  public Mono<ClashTeam> findAndRemoveUserFromTeam(String discordId, String clashTeamId) {
    return teamService
        .findTeamById(clashTeamId)
        .switchIfEmpty(Mono.error(new ClashBotDbException(MessageFormat.format("Team {0} does not exist.", clashTeamId),
                                                          HttpStatus.NOT_FOUND)))
        .flatMap(team -> removeUserFromTeam(discordId, team))
        .flatMap(userService::enrichClashTeamWithUserDetails)
        .log()
        .flatMap(team -> userAssociationService
            .delete(UserAssociationKey
                        .builder()
                        .discordId(discordId)
                        .tournamentId(team
                                          .getTeamId()
  ...
  private Mono<ClashTeam> removeUserFromTeam(String discordId, ClashTeam team) {
    ...
    if (role.isPresent()) {
      ...
      throw new ClashBotDbException(MessageFormat.format("User {0} does not belong on Team {1}.", discordId, team
          .getTeamId()
          .getId()), HttpStatus.BAD_REQUEST);
    }
    return teamService.upsertClashTeam(team);
  }

  public Mono<TentativeQueue> createTentativeQueueAndAssignUser(
      Set<String> discordIds, String serverId, String tournamentName, String tournamentDay
  ) {
    TentativeQueue baseTentativeQueue = TentativeQueue
        .builder()
    ...
    return tournamentService
        ...
        .flatMap(active -> {
          ...
          return tentativeService.doesServerAlreadyHaveATentativeQueueForTournament(serverId, tournamentName,
                                                                                    tournamentDay);
        })
        ...
        .flatMap(id -> userAssociationService
            ...
            .flatMap(assc -> {
              if (StringUtils.isNotBlank(assc.getTeamId())) {
                return teamService
                    .removeUserFromTeam(assc.getTeamId(), id)
                    .flatMap(userService::enrichClashTeamWithUserDetails)
                    .flatMap((teamAfterUpdate) -> teamSource.sendTeamRemovedEvent(
                        teamMapper.clashTeamToTeam(teamAfterUpdate)))
                    .thenReturn(assc);
              }
              return Mono.just(assc);
            })
            .log()
            .defaultIfEmpty(UserAssociation
                                .builder()
                                .serverId(serverId)
                                .userAssociationKey(UserAssociationKey
                                                        .builder()
        ...
        .collectList()
        .log()
        .flatMap(userAssociations -> tentativeService
            .createTentativeQueue(baseTentativeQueue)
            .map(createdTentativeQueue -> {
              List<UserAssociation> associations = userAssociations
                  .stream()
                  .peek(ua -> {
                    ua.setTentativeId(createdTentativeQueue
                                          .getTentativeId()
                                          .getTentativeId());
                    ua.setTeamId(null);
                  })
        ...
        .flatMap(tuple -> Flux
            .fromIterable(tuple.getT2())
            .flatMap(userAssociationService::save)
            .flatMap((userAssociation) -> teamSource
                .sendTentativeQueueCreateEvent(tentativeMapper.tentativeQueueToTentative(tuple.getT1())
                )
    ...
    return this.tentativeService
        ...
        .flatMap(tentativeQueue -> this.userAssociationService
            ...
            .flatMap(assc -> {
              if (StringUtils.isNotBlank(assc.getTeamId())) {
                return teamService
                    .removeUserFromTeam(assc.getTeamId(), discordId)
                    .flatMap(userService::enrichClashTeamWithUserDetails)
            ...
            .defaultIfEmpty(UserAssociation
                                ...
                                .serverId(tentativeQueue
                                              .getTentativeId()
                                              .getServerId())
                                .build())
            .map(assc -> Tuples.of(tentativeQueue, assc)))
        .flatMap(tuple -> this.tentativeService
            .assignUserToTentativeQueue(discordId, tuple.getT1())
        ...
        .flatMap(tuple -> this.userAssociationService
            .save(tuple.getT2())
            .flatMap((assc) -> teamSource.sendTentativeQueueJoinedEvent(
                tentativeMapper.tentativeQueueToTentative(tuple.getT1())))
            .thenReturn(tuple.getT1()));
  }
  ...
  public Mono<TentativeQueue> findAndRemoveUserFromTentativeQueue(String discordId, String tentativeQueueId) {
    return this.tentativeService
        .findById(tentativeQueueId)
        .switchIfEmpty(Mono.error(new ClashBotDbException("Tentative Queue does not exist.", HttpStatus.NOT_FOUND)))
        .flatMap(tentativeQueue -> this.tentativeService
        ...
        .flatMap(tentativeQueue -> teamSource
            .sendTentativeQueueRemovedEvent(tentativeMapper.tentativeQueueToTentative(tentativeQueue))
            .thenReturn(tentativeQueue)
        );
  }
