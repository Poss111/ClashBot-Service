...
public class TeamServiceTest {
  ...
  class Create {
    ...
    void test_createClashTeam() {
      ...
      String discordId = "1";
      Map<Role, BasePlayerRecord> positionDetailMap = Map.of(Role.TOP, BasePlayerRecord
          .builder()
          .discordId(discordId)
          .build());
      String serverId = "1234";
      TournamentId tournamentId = easyRandom.nextObject(TournamentId.class);
      ClashTeam teamToPersist = ClashTeam
          .builder()
          ...
          .teamId(TeamId
                      ...
                      .build())
          .build();
      ClashTeam teamAfterSave = ClashTeam
          .builder()
          .teamName("RandomTeamName")
          .serverId(serverId)
          .positions(positionDetailMap)
  ...
  class Query {
    ...
    @Test
    @DisplayName("retrieveTeamsBasedOnListOfTournaments - should retrieve teams based on a list of Tournaments based")
    void test10() {
      ClashTournament clashTournamentOne = easyRandom.nextObject(ClashTournament.class);
      ClashTournament clashTournamentTwo = easyRandom.nextObject(ClashTournament.class);

      ClashTeam clashTeam = easyRandom.nextObject(ClashTeam.class);
      clashTeam
          .getTeamId()
          .setTournamentId(clashTournamentOne.getTournamentId());
      ClashTeam clashTeamTwo = easyRandom.nextObject(ClashTeam.class);
      clashTeamTwo
          .getTeamId()
          .setTournamentId(clashTournamentTwo.getTournamentId());

      PublisherProbe<ClashTeam> clashTeamPublisherProbe = PublisherProbe.of(
          Flux.fromIterable(List.of(clashTeam, clashTeamTwo)));
      List<TournamentId> tournamentIds = List.of(clashTournamentOne.getTournamentId(),
                                                 clashTournamentTwo.getTournamentId());
      when(teamDao.findAllByTeamId_TournamentIdIsIn(
          tournamentIds))
          .thenReturn(clashTeamPublisherProbe.flux());

      StepVerifier
          .create(teamService.retrieveTeamsBasedOnListOfTournaments(tournamentIds))
  ...
  class Update {
    ...
    void test() {
      ...
      assertAll(() -> {
        assertEquals(1, teamEventArgumentCaptor
            .getAllValues()
            .size());
    ...
    void test2() {
      ...
      expectedPositions.put(Role.BOT, BasePlayerRecord
          .builder()
          .discordId("2")
          .build());
