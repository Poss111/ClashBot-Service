...
public class UserServiceTest {
  ...
  class Update {
    ...
    void test2() {
      ...
      updatedUser
          ...
          .put(UserSubscription.DISCORD_MONDAY_NOTIFICATION, true);
      when(userDao.findUserByDiscordId(user.getDiscordId()))
          .thenReturn(Mono.just(user));
      ...
      StepVerifier
          .create(userService.toggleUserSubscription(user.getDiscordId(), UserSubscription.DISCORD_MONDAY_NOTIFICATION,
                                                     true))
    ...
    class SelectedServerUpdates {
      ...
      void test_mergeSelectedServers_shouldMergeTheUserList_returnNewServerIdList() {
        ...
        StepVerifier
            .create(userService.mergeSelectedServers(userDiscordId, newServerIds))
      ...
      Stream<DynamicTest> testMergeSelectedServers() {
        ...
        return originalUserSelectedServerIds
            .stream()
            .map(dom -> {
              ...
              return DynamicTest.dynamicTest(
                  MessageFormat.format("Given starting Selected Servers of {0} and merging {1} should equal {2}", dom,
                                       setToMergeWith, expectedSet),
                  () -> {
                    User ogUser = User
                        .builder()
                        .selectedServers(dom)
                    ...
                    assertEquals(expectedSet, userService
                        .mergeSelectedServersList(setToMergeWith)
                        .apply(updated)
                        .getSelectedServers());
                  });
            });
    ...
    void test_overwriteSelectedServers_shouldOverwriteTheUserList_returnNewServerIdList() {
      ...
      when(userDao.save(expectedUserToSave))
          .thenReturn(Mono.just(expectedUserToSave));
      StepVerifier
          .create(userService.overwriteSelectedServers(userDiscordId, newServerIds))
          .recordWith(HashSet::new)
    ...
    Stream<DynamicTest> testOverwriteSelectedServers() {
      ...
      return originalUserSelectedServerIds
          ...
          .map(dom -> {
            ...
            Set<String> setToMergeWith = listToMergeUserSelectedServerIds.get(index);
            Set<String> expectedSet = expectedOutputSelectedServerIds.get(index);
            return DynamicTest.dynamicTest(MessageFormat.format(
                                               "Given starting Selected Servers of {0} and being overwritten with {1} it should equal {2}", dom,
                                               setToMergeWith, expectedSet),
                                           () -> {
                                             User ogUser = User
                                                 .builder()
                                             ...
                                             User updated = userMapper.clone(ogUser);
                                             assertEquals(expectedSet, userService
                                                 .overwriteSelectedServersList(setToMergeWith)
                                                 .apply(updated)
                                                 .getSelectedServers());
                                           });
          });
    ...
    void test_removeSelectedServers_shouldRemoveSelectedServersInTheUserList_returnNewServerIdList() {
      ...
      when(userDao.save(expectedUserToSave))
          .thenReturn(Mono.just(expectedUserToSave));
      StepVerifier
          .create(userService.removeSelectedServers(userDiscordId, newServerIds))
          .recordWith(HashSet::new)
          .expectNextCount(2)
    ...
    Stream<DynamicTest> testRemoveSelectedServers() {
      ...
      return originalUserSelectedServerIds
          ...
          .map(dom -> {
            ...
            Set<String> expectedSet = expectedOutputSelectedServerIds.get(index);
            return DynamicTest.dynamicTest(
                MessageFormat.format("Given starting Selected Servers of {0} and removing {1} it should equal {2}", dom,
                                     setToMergeWith, expectedSet),
                () -> {
                  User ogUser = User
                      .builder()
                  ...
                  User updated = userMapper.clone(ogUser);
                  assertEquals(expectedSet, userService
                      .removeSelectedServersList(setToMergeWith)
                      .apply(updated)
                      .getSelectedServers());
                });
          });
  ...
  class PopulateUserDetails {
    ...
    void test_enrichClashTeamsWithUserDetails_populateListOfUserDetailsForListOfClashTeams() {
      ...
      positionsTwo.put(Role.TOP, BasePlayerRecord
          .builder()
          .discordId(discordIdSix)
          .build());
      positionsTwo.put(Role.JG, BasePlayerRecord
          .builder()
          .discordId(discordIdSeven)
          .build());
      positionsTwo.put(Role.MID, BasePlayerRecord
          .builder()
          .discordId(discordIdEight)
          .build());
      positionsTwo.put(Role.BOT, BasePlayerRecord
          .builder()
          .discordId(discordIdNine)
          .build());
      positionsTwo.put(Role.SUPP, BasePlayerRecord
          .builder()
          .discordId(discordIdTen)
          .build());

      TournamentId tournamentIdTwo = TournamentId
          .builder()
          .tournamentName("awesome_sauce")
          .tournamentDay("1")
          .build();
      String serverIdTwo = "1234";
      ClashTeam clashTeamEntity = ClashTeam
          .builder()
          .teamId(TeamId
                      .builder()
                      .id(teamId)
          ...
          .positions(positions)
          .build();
      ClashTeam clashTeamEntityTwo = ClashTeam
          .builder()
          .teamId(TeamId
                      .builder()
                      .id(teamIdTwo)
          ...
          .build();
      Set<LoLChampion> expectedPreferredChamps = Set.of(LoLChampion
                                                            .builder()
                                                            .name("Anivia")
                                                            .build());
      ...
      StepVerifier
          ...
          .consumeRecordedWith(enrichedClashTeams -> {
            for (ClashTeam clashTeam : enrichedClashTeams) {
              clashTeam
                  .getPositions()
                  .forEach((key, value) -> assertEquals(expectedPreferredChamps, value.getChampionsToPlay()));
            }
          })
          .verifyComplete();
    }
    ...
    void test_populateUserClashTeamDetails() {
      ...
      positions.forEach((key, value) -> idToPlayerDetailsMap.put(value.getDiscordId(), User
          .builder()
          .discordId(value.getDiscordId())
          .preferredChampions(expectedPreferredChamps)
          .build()));
      ClashTeam populatedTeam = userService.populateTeamUserDetails(clashTeamEntity, idToPlayerDetailsMap);
      assertNotNull(populatedTeam
                        .getPositions()
                        .get(Role.TOP)
                        .getChampionsToPlay());
      assertEquals(expectedPreferredChamps, populatedTeam
          .getPositions()
          .get(Role.TOP)
          .getChampionsToPlay());
    }
