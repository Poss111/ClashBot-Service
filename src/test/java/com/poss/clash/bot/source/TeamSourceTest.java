...
public class TeamSourceTest {
    ...
    class TeamEvents {
        ...
        Stream<DynamicTest> testTeamEvents() {
            ...
            team.getPlayerDetails().getTop().setChampions(List.of());
            team.getPlayerDetails().getJg().setChampions(List.of());
            team.getPlayerDetails().getMid().setChampions(List.of());
            team.getPlayerDetails().getBot().setChampions(List.of());
            team.getPlayerDetails().getSupp().setChampions(List.of());
            ...
            return testInput.stream()
                    .map(dom -> DynamicTest.dynamicTest(MessageFormat.format(
                            ...
                            () -> {
                                ...
                                StepVerifier
                                        .create(publisher,
                                                StepVerifierOptions.create().withInitialContext(Context.of(CAUSED_BY_KEY, dom.getT1())))
                                        .expectNextMatches(eventPublished -> {
                                            assertNotNull(eventPublished.getId());
                                            assertEquals(inputTeam, event.getTeamEvent().getTeam());
    ...
    class TentativeEvents {
        ...
        Stream<DynamicTest> testTentativeEvents() {
            ...
            List<Tuple4<String, EventType, Tentative, String>> testInput = List.of(
                    Tuples.of(causedById, EventType.JOINED, tentative, causedById + " joined Tentative Queue for " + tentative.getTournamentDetails().getTournamentName() + " " + tentative.getTournamentDetails().getTournamentDay()),
                    Tuples.of(causedById, EventType.CREATED, tentative, causedById + " created Tentative Queue for " + tentative.getTournamentDetails().getTournamentName() + " " + tentative.getTournamentDetails().getTournamentDay()),
                    Tuples.of(causedById, EventType.REMOVED, tentative, causedById + " removed from Tentative Queue for " + tentative.getTournamentDetails().getTournamentName() + " " + tentative.getTournamentDetails().getTournamentDay())
            );
            ...
            return testInput.stream()
                    .map(dom -> DynamicTest.dynamicTest(MessageFormat.format(
                            ...
                            () -> {
                                ...
                                switch(dom.getT2()) {
                                    case JOINED:
                                        publisher = teamSource.sendTentativeQueueJoinedEvent(inputTentative);
                                        break;
                                    case CREATED:
                                        publisher = teamSource.sendTentativeQueueCreateEvent(inputTentative);
                                        break;
                                    case REMOVED:
                                        publisher = teamSource.sendTentativeQueueRemovedEvent(inputTentative);
                                        break;
                                    default:
                                        fail("Event type not supported.");
                                }
                                StepVerifier
                                        ...
                                        .expectNextMatches(eventPublished -> {
                                            assertNotNull(eventPublished.getId());
                                            assertEquals(inputTentative, event.getTeamEvent().getTentative());
                                            assertEquals(event.getTeamEvent(), eventPublished.getTeamEvent());
                                            assertEquals(event.getCausedBy(), eventPublished.getCausedBy());
                                            assertEquals(event.getSummary(), eventPublished.getSummary());
                                            return true;
